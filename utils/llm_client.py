...
class LLMClient:
    ...
    def __init__(
        ...
        self.token_counter = TokenCounter(self.model if provider == "openai" else "gpt-4")
        
        if self.verbose:
            logger.setLevel(logging.DEBUG)
        
        logger.debug(f"Initialized LLM client with provider: {provider}, model: {self.model}")
    ...
    def call(
        ...
        if self.provider == "openai":
            ...
            if token_count + max_tokens > 8192 and "gpt-4" in self.model:
                logger.warning(f"Token count ({token_count} + {max_tokens}) approaching GPT-4 limit (8192)")
            elif token_count + max_tokens > 4096 and "gpt-3.5" in self.model:
                logger.warning(f"Token count ({token_count} + {max_tokens}) approaching GPT-3.5 limit (4096)")
        
        try:
            start_time = time.time()
            response = providers[self.provider](prompt, max_tokens, temperature, system_message)
